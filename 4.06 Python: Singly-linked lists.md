...
The ``insert_after()`` method assigns the new node's next data member to the existing node's next data member.   
The ``insert_after()`` method assigns the existing node's next data member to the new node.   
